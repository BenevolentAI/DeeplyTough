...
def get_cli_args():
    ...
    parser.add_argument('--db_exclude_vertex', default='', type=str, help='Whether to exclude Vertex dataset proteins in the training fold: (|uniprot|pdb)')
    ...
    parser.add_argument('--db_split_strategy', default='uniprot_folds', help="pdb_folds|uniprot_folds|none")
...
def main():
    ...
    train_dataset, test_dataset = create_tough_dataset(args, fold_nr=args.cvfold, n_folds=args.num_folds, seed=args.seed-1, exclude_Vertex_from_train=args.db_exclude_vertex, exclude_Prospeccts_from_train=args.db_exclude_prospeccts)
    logger.info('Train set size: %d, test set size: %d', len(train_dataset), len(test_dataset))

    # Create model and optimizer
    if args.resume != '':
        if args.resume=='RESUME': args.resume = args.output_dir + '/model.pth.tar'
    ...
    def train():
        ...
        loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size//args.batch_parts, num_workers=args.nworkers, shuffle=True, drop_last=True, worker_init_fn=set_worker_seed)
        ...
        for bidx, batch in enumerate(loader):
            if 0 < args.max_train_samples < bidx * args.batch_size//args.batch_parts: break
    ...
    def test():
        ...
        loader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size//args.batch_parts, num_workers=args.nworkers, worker_init_fn=set_worker_seed)
        if logging.getLogger().getEffectiveLevel() > logging.DEBUG: loader = tqdm(loader, ncols=100)
        ...
        with torch.no_grad():
            for bidx, batch in enumerate(loader):
                if 0 < args.max_test_samples < bidx * args.batch_size//args.batch_parts: break
    ...
    for epoch in range(args.start_epoch, args.epochs):
        print('Epoch {}/{} ({}):'.format(epoch, args.epochs, args.output_dir))
        scheduler.step()

        train_stats = train()
        for k,v in train_stats.items(): writer.add_scalar('train/' + k, v, epoch)
        print('-> Train distances: p {}, n {}, \tLoss: {}'.format(train_stats['pos_dist'], train_stats['neg_dist'], train_stats['loss']))

        if (epoch+1) % args.test_nth_epoch == 0 or epoch+1==args.epochs:
            test_stats = test()
            for k,v in test_stats.items(): writer.add_scalar('test/' + k, v, epoch)
            print('-> Test distances: p {}, n {}, \tLoss: {}'.format(test_stats['pos_dist'], test_stats['neg_dist'], test_stats['loss']))

        torch.save({'epoch': epoch + 1, 'args': args, 'state_dict': model.state_dict(), 'optimizer' : optimizer.state_dict(), 'scheduler': scheduler.state_dict()},
                   os.path.join(args.output_dir, 'model.pth.tar'))

        if math.isnan(train_stats['loss']): break
...
def estimate_scaler(args, train_dataset, nsamples):
    ...
    with tqdm(total=nsamples) as pbar:
        while True:
            loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, num_workers=args.nworkers, shuffle=True, drop_last=True, worker_init_fn=set_worker_seed)
            for batch in loader:
